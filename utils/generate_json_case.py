...
def get_neighbors(unit_index: int, rows: int, columns: int):
    ...
    for neighbor_index in range(1, max_possible_neighbors + 1):
        ...
        if neighbor_index == 1:
            # up left
            neighbor_id = unit_index - columns - 1
            neighbor_id = neighbor_id if neighbor_id >= (unit_row_index - 2) * columns + 1 else 0
        elif neighbor_index == 2:
            neighbor_id = unit_index - columns  # up
        elif neighbor_index == 3:
            neighbor_id = unit_index - columns + 1  # up right
            neighbor_id = neighbor_id if neighbor_id <= (unit_row_index - 1) * (columns) else 0
        elif neighbor_index == 4:
            # right
            neighbor_id = unit_index + 1 if unit_index + 1 <= unit_row_index * columns else 0
        elif neighbor_index == 5:
            neighbor_id = unit_index + columns + 1  # down right
            neighbor_id = neighbor_id if neighbor_id <= (unit_row_index + 1) * columns else 0
        elif neighbor_index == 6:
            neighbor_id = unit_index + columns  # down
        elif neighbor_index == 7:
            neighbor_id = unit_index + columns - 1  # down left
            neighbor_id = neighbor_id if neighbor_id >= unit_row_index * columns + 1 else 0
...
if __name__ == "__main__":
    with open("data_5_5.json", "w") as file:
        json.dump(generate_json_object(5, 5), file)
